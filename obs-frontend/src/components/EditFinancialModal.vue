    ...
    <div class="modal-container">
      ...
      <form @submit.prevent="handleSubmit" class="modal-form">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <label for="vencimentDay">Data de Vencimento *</label>
            <input
              type="date"
              id="vencimentDay"
              v-model="formData.vencimentDay"
              required
            />
          </div>
        </div>
...
export default {
  ...
  watch: {
    financial: {
      handler(newFinancial) {
        if (newFinancial) {
          this.formData = {
            accountName: newFinancial.accountName,
            price: parseFloat(newFinancial.price).toFixed(2).replace('.', ','),
            vencimentDay: newFinancial.vencimentDay.split('T')[0],
            paymentStatus: newFinancial.paymentStatus
          };
        }
      },
      immediate: true
  ...
  methods: {
    ...
    async handleSubmit() {
      try {
        ...
        if (!this.financial || !this.financial.id) {
        ...
        const response = await fetch(`http://localhost:3000/financials/${this.financial.id}`, {
          method: 'PATCH',
          headers: {
        ...
        this.success = true;
        this.$emit('financial-updated', data);
