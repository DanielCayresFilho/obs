    ...
    async handleSubmit() {
      try {
        ...
        if (!this.procedure || !this.procedure.id) {
          throw new Error('ID do procedimento não encontrado');
        }
        ...
        const response = await fetch(`http://localhost:3000/procedures/${this.procedure.id}`, {
          method: 'PATCH',
          headers: {
        ...
        this.$emit('procedure-updated', data);
